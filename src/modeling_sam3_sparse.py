...
"""transformers 5.0.0rc1 的 models/sam3/modeling_sam3.py 的 fork。

基準:transformers 5.0.0rc1,官方檔 2395 行。升版時用 tools/difffork.sh 重新對照。

=== 給 code reviewer ===
每一處增刪改都用固定文法標註,搜尋 "ROI-PRUNE" 即可逐處檢視:

    # ┌─ ROI-PRUNE <id> ─ <ADDED|MODIFIED|DELETED> ─ 官方 modeling_sam3.py:<行號>
    # │ 原本:...
    # │ 改成:...
    # │ 理由:...
    # └─ ROI-PRUNE <id> END

機械式 diff:bash tools/difffork.sh
標註完整性由 tests/test_parity.py::test_every_fork_change_is_annotated_for_review 檢查。

改動清單(見 docs/design.md 3.2):
  a. Sam3ViTRoPEAttention.forward  接受 [B,S,C]、attention_mask、外部 cos/sin
  b. Sam3ViTLayer.forward          移除 window_partition;sparse 走遮罩路徑
  c. Sam3ViTModel.forward          gather / scatter_add
  d. Sam3VisionModel.__init__      backbone 改用本模組的 Sam3ViTModel
     Sam3VisionModel.forward / Sam3Model.forward / get_vision_features  穿 roi
  e. Sam3DetrEncoder / Sam3DetrDecoder  key-only 遮罩

**未逐行標註的機械性改動**:檔頭全部 import 從相對(`from ...utils import`)
改成絕對(`from transformers.utils import`),因為本檔不在 transformers package 內。
行為零影響。

roi=None 時逐元素等於官方模型 —— tests/test_parity.py 守著這件事。
"""
...
# ┌─ ROI-PRUNE a ─ MODIFIED ─ 官方 modeling_sam3.py:502-534
# │ 原本:hidden_states 是 [B,H,W,C],內部解成 (batch, height, width, _);
# │      attention_interface 的 attention_mask 寫死成 None;
# │      輸出 reshape 成 (batch_size, height, width, -1)。
# │ 改成:hidden_states 是扁平的 [B,S,C];新增 attention_mask 參數;
# │      輸出 reshape 成 (batch_size, seq_len, -1)。
# │ 理由:讓 28 層 windowed(塊對角遮罩 + window 內 RoPE)與 4 層 global
# │      (全通遮罩 + 全域 RoPE)共用同一條路徑。密集路徑的 [B,H,W,C]
# │      進出 reshape 移到 Sam3ViTLayer(改動 b),數值不變。
# │      見 docs/cost_model.md 5.1、docs/design.md 3.2。
    def forward(
        self,
...
# ┌─ ROI-PRUNE b ─ MODIFIED ─ 官方 modeling_sam3.py:734-761
# │ 原本:單一路徑。window_size>0 時做 window_partition → attention →
# │      window_unpartition,全程用 [B,H,W,C];RoPE 直接用 self.rotary_emb()
# │      的完整 buffer。
# │ 改成:新增 roi 參數。roi=None 走原本的密集邏輯(額外在 attention 進出各做
# │      一次 reshape,因為改動 a 讓 attention 只吃 [B,S,C] —— 數值不變);
# │      roi 非 None 時跳過 window_partition,直接對 [B,N,C] 做 attention,
# │      並帶入 index_select 過的 RoPE 與逐層遮罩。
# │ 理由:gather 之後 token 是扁平序列,window 隔離要靠遮罩還原;而 windowed
# │      層的 RoPE 位置跨 window 重複((5,5) 與 (29,29) 同位置),不隔離的話
# │      位置編碼是錯的。見 docs/cost_model.md 5.1。
    def forward(
        self,
        hidden_states: torch.Tensor,
        roi=None,
        **kwargs: Unpack[TransformersKwargs],
        ...
        """roi=None 時 hidden_states 是 [B,H,W,C](官方語意);否則是 [B,N,C]。"""
        residual = hidden_states
        hidden_states = self.layer_norm1(hidden_states)
        is_global = self.window_size == 0
        ...
        if roi is None:
            ...
            batch, win_h, win_w, channels = hidden_states.shape
            cos, sin = self.rotary_emb()
# ┌─ ROI-PRUNE h ─ ADDED ─ 官方 modeling_sam3.py:748 之後
# │ 原本:global 層直接用完整的 72×72 RoPE buffer([5184,64]),因為序列一定是 5184。
# │ 改成:序列比 buffer 短時(策略 C 把 grid 裁成前 rect_rows 個 row)切前 seq 個。
# │ 理由:rows 0..rect_rows-1 × **整寬** 的 flatten 索引剛好是 0..seq-1 **連續**,
# │      所以 cos[:seq] 就是正確的位置編碼 —— 這個等式只在「從 row 0 開始裁、
# │      且保留整個寬度」時成立,也正是 letterbox 黑邊在下方的情形。
# │      seq == buffer 長度時是 no-op,所以既有密集路徑逐元素不變。
# │      windowed 層不受影響:partition 之後序列恆為 window²,與 buffer 同長。
            seq = win_h * win_w
            if is_global and cos.shape[0] != seq:
                cos, sin = cos[:seq], sin[:seq]
# └─ ROI-PRUNE h END
            attn_out, _ = self.attention(
                hidden_states.reshape(batch, win_h * win_w, channels),
                (cos, sin), None, **kwargs)
            hidden_states = attn_out.reshape(batch, win_h, win_w, channels)
            if not is_global:
                hidden_states = window_unpartition(
                    hidden_states, self.window_size, pad_height_width,
                    (height, width))
        else:
            # 每層用自己的 RoPE buffer,不假設各層 buffer 相同。
            cos_full, sin_full = self.rotary_emb()
            sel = roi.rope_idx(is_global)
            hidden_states, _ = self.attention(
                hidden_states, (cos_full[sel], sin_full[sel]),
                roi.vit_mask(is_global), **kwargs)
...
# ┌─ ROI-PRUNE c ─ MODIFIED ─ 官方 modeling_sam3.py:801-823
# │ 原本:embeddings → view 成 [B,H,W,C] → layer_norm → 32 層 → view 回
# │      [B,H*W,C]。無 roi 概念。
# │ 改成:新增 roi 參數。roi=None 時逐行等於官方(只多傳 roi=None 給每層);
# │      roi 非 None 時在 layer_norm 後 gather 成 [B,N,C]、跑完 32 層、
# │      歸零 dummy、再 scatter_add 回 [B,5184,C]。
# │ 理由:這是「真的少算」的所在 —— 張量維度真的變小,不是遮蔽。
# │      scatter_add 而非 scatter:dummy 索引全是 0,重複索引的寫入順序未定義,
# │      plain scatter 會覆蓋真實 token 0;先歸零則重複加的是 0、自動無害。
# │      見 docs/cost_model.md 4.5、docs/design.md 3.2。
    ...
    def forward(
        ...
        pixel_values: torch.Tensor,
        roi=None,
        rect_rows=None,          # ROI-PRUNE h:策略 C,矩形裁切
        rect_fill=None,          # ROI-PRUNE i:缺席列填什麼
        **kwargs: Unpack[TransformersKwargs],
    ) -> BaseModelOutput:
        r"""
        roi (`RoiContext`, *optional*):
            If given, gather only the ROI's tokens through the ViT layers and scatter the
            result back into the full `[B, height*width, hidden]` layout with dropped
            positions zeroed; if `None`, run the original dense per-window path unchanged.
                rect_rows (`int`, *optional*):
            ROI-PRUNE h(策略 C):只保留 token grid 的前 rect_rows 個 row,
            其餘補零。用於 letterbox 去黑邊 —— ROI 是矩形時不需遮罩,
            沿用官方 window_partition/global attention,FlashAttention 保得住。
            與 roi 可並用:ViT 走矩形裁切,roi 只由 DETR 側消費。
        """
        hidden_states = self.embeddings(pixel_values)  # [B, seq_len, hidden]
...
# ┌─ ROI-PRUNE h ─ ADDED ─ 官方 modeling_sam3.py:806-823 之間
# │ 原本:32 層固定跑在完整的 [B,72,72,C] 上,只有 roi 一種稀疏路徑。
# │ 改成:rect_rows 非 None 時走「矩形裁切」路徑 —— 只保留前 rect_rows 個 token row,
# │      跑完補零回 72 row。**必須放在 `if roi is None:` 之前**:策略 C 會同時傳
# │      roi(給 DETR 側 gather)與 rect_rows(給 ViT),若放在 roi is None 分支內,
# │      rect_rows 會被靜默忽略、退回策略 A —— 這個錯誤實際發生過,症狀是 C 的
# │      mAP 與 latency 與 A **逐位元相同**。
# │ 理由:策略 C(去黑邊專用)。ROI 是矩形時不需要 gather/遮罩,直接裁 grid,
# │      而且沿用官方 attention:window_partition 會把 41 row pad 到 48 切成 2×3=6 個
# │      window(最下面那排全黑邊的 3 個自動不算),global 層靠改動 h 的 cos[:seq]。
# │      沒有遮罩 → FlashAttention 保得住。對照:策略 A 把 28 層 windowed 變成
# │      dense N²,在 N=2952 比官方貴 328 GMAC;C 是 114.2 vs 官方 171.2。
        if rect_rows is not None:
            hidden_states = hidden_states.view(batch_size, height, width, hidden_size)
            hidden_states = self.layer_norm(hidden_states)[:, :rect_rows]
            for layer in self.layers:
                hidden_states = layer(hidden_states, roi=None, **kwargs)
            dense = hidden_states.new_zeros(batch_size, height, width, hidden_size)
            dense[:, :rect_rows] = hidden_states
# ┌─ ROI-PRUNE i ─ ADDED ─ 承接改動 h
# │ 原本(改動 h):缺席的 row 一律留 0。
# │ 改成:rect_fill 可選 —— None 補零(原行為),"replicate" 複製最後一列內容,
# │      或給一個 [n_missing, width, C] 的張量(用官方 dense 路徑算出的黑邊特徵平均)。
# │ 理由:0 是分布外的值,而 FPN proj2 的 3×3 卷積會把它混進邊界內容列。
# │      三種填充只差在下游 FPN 邊界,ViT 的計算量完全相同 —— 所以這是一組
# │      控制了速度的乾淨對照,單獨檢驗「填什麼」是否重要。
            if rect_fill is not None:
                if rect_fill == "replicate":
                    dense[:, rect_rows:] = hidden_states[:, -1:]
                else:
                    dense[:, rect_rows:] = rect_fill.to(dense.dtype)
# └─ ROI-PRUNE i END
            return BaseModelOutput(
                last_hidden_state=dense.view(batch_size, height * width, hidden_size))
# └─ ROI-PRUNE h END
        ...
        # ROI-PRUNE c 延伸:B>1 呼叫帶著 B=1 的 RoiContext 時,torch.gather
        # 用 [1,N,C] 索引對 [B,...] 輸入不會報錯,只回傳 [1,N,C],下面的
        # scatter_add 因此只會寫回 batch 0,其餘 batch 靜默留零 ——
        # 沒有 NaN、沒有例外,連邊界測試的有限性斷言都會通過。本專案明確
        # 只支援 B=1(design.md 2.1),所以在 gather 之前就把「不支援的組態」
        # 變成立即的錯誤,而不是讓它變成靜默的部分計算。
        assert roi.token_idx.shape[0] == batch_size, (
            f"roi.token_idx 的 batch={roi.token_idx.shape[0]} 與輸入 batch="
            f"{batch_size} 不一致;B>1 配 B=1 的 RoiContext 時 gather/"
            f"scatter_add 不會報錯,只會讓多出的 batch 靜默算出全零特徵。"
            f"本專案只支援 B=1(design.md 2.1)。"
        )

        # Sparse:LayerNorm 只作用於最後一維,所以在 [B,5184,C] 上做與官方
        # 在 [B,H,W,C] 上做逐元素相同。
        hidden_states = self.layer_norm(hidden_states)
        gather_idx = roi.token_idx[..., None].expand(-1, -1, hidden_size)
        hidden_states = torch.gather(hidden_states, 1, gather_idx)  # [B,N,C]

        for layer in self.layers:
            hidden_states = layer(hidden_states, roi=roi, **kwargs)

        # 先歸零 dummy,再 scatter_add:dummy 的索引都是 0,重複索引加的是 0
        # 就自動無害,不會覆蓋真實 token 0。見 cost_model.md 4.5。
        hidden_states = hidden_states * roi.valid[..., None].to(hidden_states.dtype)
        dense = torch.zeros(
            batch_size, height * width, hidden_size,
            dtype=hidden_states.dtype, device=hidden_states.device)
        hidden_states = dense.scatter_add(1, gather_idx, hidden_states)
        return BaseModelOutput(last_hidden_state=hidden_states)
# └─ ROI-PRUNE c END
...
class Sam3VisionModel(Sam3PreTrainedModel):
    ...
    def __init__(self, config: Sam3VisionConfig):
        super().__init__(config)
        self.config = config
# ┌─ ROI-PRUNE d ─ MODIFIED ─ 官方 modeling_sam3.py:1013
# │ 原本:self.backbone = AutoModel.from_config(config.backbone_config)
# │ 改成:直接建構本模組的 Sam3ViTModel
# │ 理由:AutoModel 會依 config 解析到**官方**的 Sam3ViTModel,於是本模組的
# │      sparse 路徑永遠不會被執行,而每個對帳測試都會因為跑了密集路徑而
# │      「通過」。test_fork_backbone_is_our_class_not_the_official_one 守著它。
...
# ┌─ ROI-PRUNE d ─ MODIFIED ─ 官方 modeling_sam3.py:1022,1030
# │ 原本:forward(self, pixel_values=None, **kwargs);
# │      backbone_output = self.backbone(pixel_values, **kwargs)
# │ 改成:簽章多一個 roi=None,並轉交給 backbone。
# │ 理由:純轉接。roi 得從 Sam3Model 一路穿到 Sam3ViTModel(改動 c)。
        backbone_output = self.backbone(pixel_values, roi=roi, rect_rows=rect_rows, rect_fill=rect_fill, **kwargs)
# └─ ROI-PRUNE d END
...
class Sam3DetrEncoderLayer(nn.Module):
    ...
    def forward(
        ...
        vision_key_mask: Optional[Tensor] = None,  # ROI-PRUNE f:已不再由 encoder 傳入,保留以免破壞介面
        **kwargs: Unpack[TransformersKwargs],
    ):
        """
        ...
        Args:
            ...
            vision_key_mask: Additive key-only mask for vision self-attention,
                shape [batch_size, 1, 1, vision_len]
...
# ┌─ ROI-PRUNE e ─ MODIFIED ─ 官方 modeling_sam3.py:1267,1291-1296
# │ 原本:簽章無遮罩參數;self_attn 呼叫不傳 attention_mask(即 None)。
# │ 改成:簽章多 vision_key_mask,並傳進 self_attn 的 attention_mask。
# │ 理由:非 ROI 位置離開 ViT 時是零特徵,卻帶著合法的 sine 位置編碼,
# │      DETR 會在那裡生出垃圾偵測。見 docs/design.md 3.2、cost_model.md 8。
        hidden_states, _ = self.self_attn(
            query=hidden_states_with_pos,
            key=hidden_states_with_pos,
            value=hidden_states,
            attention_mask=vision_key_mask,
            **kwargs,
        )
# └─ ROI-PRUNE e END
        hidden_states = self.dropout(hidden_states) + residual
...
# ┌─ ROI-PRUNE e ─ ADDED ─ 官方 modeling_sam3.py:1390 之後
# │ 原本:簽章沒有這個參數。
# │ 改成:新增 vision_key_invalid,供下面建構 key-only 遮罩用。
# │ 理由:純介面擴充,實際遮罩建構與套用見下方兩處 ROI-PRUNE e 區塊。
        vision_key_invalid: Optional[torch.Tensor] = None,
# └─ ROI-PRUNE e END
# ┌─ ROI-PRUNE f ─ ADDED ─ 官方 modeling_sam3.py:1390 之後
# │ 原本:無。
# │ 改成:新增 roi,讓 encoder 能 gather vision token 而不是遮罩它們。
# │ 理由:profiling 顯示傳 attn_mask 會讓 SDPA 從 FlashAttention 掉到
# │      mem-efficient 後端(flash 41.1ms/120 calls → 0;memeff 19.7→141.1ms),
# │      每幀約 +8ms。遮罩的算術本身不貴,貴的是後端退化,所以只能拿掉遮罩。
# │      gather 之後非 ROI 的 key 不存在,自然不需要遮罩,且 self-attn 從
# │      5184² 降為 N²。見 docs/report.md 階段 4 的 profiling 小節。
        roi=None,
# └─ ROI-PRUNE f END
        **kwargs: Unpack[TransformersKwargs],
    ):
# ┌─ ROI-PRUNE e ─ MODIFIED ─ 官方 modeling_sam3.py:1401(docstring)
# │ 原本:docstring 沒有 vision_key_invalid 的 Args 條目(官方沒有這個參數)。
# │ 改成:補上對應的 Args 條目。純文字,不改任何邏輯。
        """
        ...
        Args:
            ...
            vision_key_invalid: Optional bool mask [batch_size, 1, 1, vision_len],
                True where the key must be masked out (non-ROI positions)
...
# ┌─ ROI-PRUNE f ─ MODIFIED ─ 官方 modeling_sam3.py:1422-1445
# │ 原本:6 層 encoder 直接在密集的 [B,5184,C] 上跑,沒有任何 vision 遮罩。
# │      (ROI-PRUNE e 曾在此加一個 key-only 加法遮罩,現已移除。)
# │ 改成:roi 非 None 時先 gather 成 [B,N,C] 跑完 6 層、再 scatter_add 回
# │      [B,5184,C];層呼叫**不再傳任何 vision 遮罩**。
# │ 理由:遮罩會讓 SDPA 掉出 FlashAttention(每幀約 +8ms,見 f 的簽章標註)。
# │      gather 之後非 ROI 的 key 不存在,語意與遮罩相同但更強 —— 非 ROI 位置
# │      的輸出從「垃圾值(靠 decoder 遮掉)」變成「恰好 0」。decoder 端的
# │      遮罩仍保留(它的 RPB 本來就強制 mem-efficient,加遮罩不再有額外成本)。
# │      pos_embeds_flattened 必須回傳**密集版**給 decoder,只有進 layer 的
# │      那一份 gather —— 否則 decoder 會拿到 N 長度的 pos 配 5184 長度的特徵。
        dense_len = features_flattened.shape[1]
        gather_idx = None
        pos_for_layers = pos_embeds_flattened
        if roi is not None:
# ┌─ ROI-PRUNE g ─ MODIFIED ─ 修正 ROI-PRUNE f 引入的缺陷
# │ 原本(f 的第一版):用**完整**的 roi.token_idx(長度 = budget)gather,所以
# │      budget > n_valid 時,dummy 槽位會 gather 到 token 0 的特徵,而 6 層 encoder
# │      **完全沒有遮罩**,這些 dummy 就以「token 0 的複本」身分參與 self-attention。
# │      歸零只發生在層跑完之後,太晚了。budget=5184 / n_valid=2952 時等於有 2232 份
# │      token 0 的複本污染 attention —— 實測讓 COCO det50 差了 0.028。
# │ 改成:只 gather 前 n_valid 個(切片),dummy 根本不進 encoder。
# │ 理由:這樣既正確又**不需要遮罩**,所以保住 FlashAttention(見 f 的說明:
# │      傳 attn_mask 會讓 SDPA 退到 mem-efficient,每幀 +8ms);而且 budget > n_valid
# │      時比原本更快。ViT 那側(ROI-PRUNE c)本來就用 m_win/m_glb 正確遮蔽 dummy,
# │      不受此缺陷影響 —— 只有 encoder 漏了。
# │      ⚠️ int(roi.n_valid[0]) 是一次 GPU→CPU 同步、且產生 data-dependent shape,
# │        對 torch.export/TRT 不友善。階段 6 匯出時 budget 本來就會等於 n_valid,
# │        屆時這個切片是恆等變換,可用 budget 靜態值取代。
            n_valid = int(roi.n_valid[0])
            gather_idx = roi.token_idx[:, :n_valid, None].expand(
                -1, -1, features_flattened.shape[-1])
            features_flattened = torch.gather(features_flattened, 1, gather_idx)
            pos_for_layers = torch.gather(pos_embeds_flattened, 1, gather_idx)
# └─ ROI-PRUNE g END
        ...
        for layer in self.layers:
            hidden_states = layer(
                ...
                vision_pos_encoding=pos_for_layers,
                prompt_cross_attn_mask=prompt_cross_attn_mask,
                **kwargs,
            )

        if roi is not None:
# ┌─ ROI-PRUNE g ─ MODIFIED ─ 承上
# │ 原本:hidden_states 有 budget 個槽位,先用 roi.valid 歸零 dummy 再 scatter_add。
# │ 改成:切片之後只有 n_valid 個槽位、沒有 dummy,所以不需要歸零;
# │      gather_idx 也已經是 n_valid 長度,scatter_add 索引不重複。
            dense = torch.zeros(
                hidden_states.shape[0], dense_len, hidden_states.shape[-1],
                dtype=hidden_states.dtype, device=hidden_states.device)
            hidden_states = dense.scatter_add(1, gather_idx, hidden_states)
# └─ ROI-PRUNE g END
# └─ ROI-PRUNE f END
...
# ┌─ ROI-PRUNE e ─ ADDED ─ 官方 modeling_sam3.py:1683 之後
# │ 原本:簽章沒有這個參數。
# │ 改成:新增 vision_key_invalid,供下面建構 key-only 遮罩用。
# │ 理由:純介面擴充,實際遮罩建構與疊加到 RPB 上見下方兩處 ROI-PRUNE e 區塊。
        vision_key_invalid: Optional[torch.Tensor] = None,
# └─ ROI-PRUNE e END
        **kwargs: Unpack[TransformersKwargs],
    ) -> tuple[torch.Tensor, torch.Tensor, Optional[torch.Tensor], Optional[torch.Tensor]]:
# ┌─ ROI-PRUNE e ─ MODIFIED ─ 官方 modeling_sam3.py:1693(docstring)
# │ 原本:docstring 沒有 vision_key_invalid 的 Args 條目(官方沒有這個參數)。
# │ 改成:補上對應的 Args 條目。純文字,不改任何邏輯。
        """
        ...
        Args:
            ...
            vision_key_invalid: Optional bool mask [batch_size, 1, 1, height*width],
                True where the vision key must be masked out (non-ROI positions)
...
# ┌─ ROI-PRUNE e ─ ADDED ─ 官方 modeling_sam3.py:1734 之後
# │ 原本:無。vision_cross_attn_mask 只承載 RPB(relative position bias)。
# │ 改成:在 RPB 之上疊加 ROI 遮罩。
# │ 理由:RPB 已經佔用了 attention_mask 這個參數位置,所以必須**疊加**而非
# │      取代。用 masked_fill 而不是相加:fp16 下 RPB + finfo.min 會溢出成
# │      -inf,而 -inf 可能在部分 kernel 產生 NaN。見 docs/design.md 5.1。
            if vision_key_invalid is not None:
                neg = torch.finfo(vision_cross_attn_mask.dtype).min
                vision_cross_attn_mask = vision_cross_attn_mask.masked_fill(
                    vision_key_invalid, neg)
# └─ ROI-PRUNE e END
...
# ┌─ ROI-PRUNE d ─ MODIFIED ─ 官方 modeling_sam3.py:2169,2200
# │ 原本:get_vision_features(self, pixel_values, **kwargs);docstring 沒有 roi 的
# │      Args 條目;vision_outputs = self.vision_encoder(pixel_values, **kwargs)
# │ 改成:簽章多一個 roi=None,docstring 補上 roi 的 Args 條目(@auto_docstring
# │      對簽章裡沒被 docstring 或標準登記表涵蓋的參數,每次呼叫都印一行
# │      [ERROR] 日誌;roi 不在標準登記表裡,得自己補),並轉交下去。
# │ 理由:純轉接。這是 docs/design.md 2.2 範例裡的 vision 快取入口。
        r"""
        roi (`RoiContext`, *optional*):
            If given, only the ROI's tokens are computed through the vision encoder and the
            rest of the `[B, height*width, hidden]` output is zero; if `None`, this is the
            original dense path with all tokens computed.
        ...
        ```
            ...
            與 roi 可並用:ViT 走矩形裁切,roi 只由 DETR 側消費。
        """
        vision_outputs = self.vision_encoder(pixel_values, roi=roi, rect_rows=rect_rows, rect_fill=rect_fill, **kwargs)
# └─ ROI-PRUNE d END
...
# ┌─ ROI-PRUNE d ─ MODIFIED ─ 官方 modeling_sam3.py:2211(docstring)
# │ 原本:forward 的簽章與 docstring 都沒有 roi(官方沒有這個參數)。
# │ 改成:docstring 補上 roi 的 Args 條目,對應簽章新增的 roi=None
# │      (簽章本身的變動見下面呼叫 vision_encoder 那個 ROI-PRUNE d 區塊)。
# │ 理由:@auto_docstring 對簽章裡沒被 docstring 或標準登記表涵蓋的參數,
# │      每次呼叫都印一行 [ERROR] 日誌;roi 不在標準登記表裡,得自己補。
        r"""
        roi (`RoiContext`, *optional*):
            If given, only the ROI's vision tokens participate in the DETR encoder/decoder
            cross-attention (masked via `roi.key_invalid`); if `None`, this is the original
            dense path with every vision token attended to.
...
# ┌─ ROI-PRUNE d ─ MODIFIED ─ 官方 modeling_sam3.py:2205,2262-2265
# │ 原本:forward 簽章無 roi;vision_outputs = self.vision_encoder(pixel_values, **kwargs)
# │ 改成:簽章多 roi=None,並轉交給 vision_encoder。
# │ 理由:純轉接,這是整條 roi 鏈的入口。
...
# ┌─ ROI-PRUNE e ─ MODIFIED ─ 官方 modeling_sam3.py:2335-2341
# │ 原本:detr_encoder 呼叫沒有 vision_key_invalid。
# │ 改成:roi 非 None 時傳入 roi.key_invalid。
# │ 理由:見改動 e 在 Sam3DetrEncoder 的說明。
        encoder_outputs = self.detr_encoder(
            ...
            vision_key_invalid=None,  # ROI-PRUNE f:encoder 改用 gather,不再需要遮罩
            roi=roi,
            **kwargs,
        )
# └─ ROI-PRUNE e END

# ┌─ ROI-PRUNE e ─ MODIFIED ─ 官方 modeling_sam3.py:2343-2349
# │ 原本:detr_decoder 呼叫沒有 vision_key_invalid。
# │ 改成:roi 非 None 時傳入 roi.key_invalid。
# │ 理由:decoder 的 200 個 query 會 cross-attend 到全部 5184 個 vision token,
# │      非 ROI 那些是零特徵,不遮會生出垃圾偵測。遮罩在 decoder 內部疊加到
# │      RPB 上(見 Sam3DetrDecoder.forward 的 ROI-PRUNE e)。
        decoder_outputs = self.detr_decoder(
            ...
            vision_key_invalid=None if roi is None else roi.key_invalid,
            **kwargs,
        )
# └─ ROI-PRUNE e END
...
# ┌─ ROI-PRUNE d ─ ADDED ─ 官方 modeling_sam3.py:檔尾
# │ 原本:無此別名
# │ 改成:給 sparse 用法一個不會與官方 Sam3Model 混淆的名字
# │ 理由:docs/design.md 2.2 的呼叫範例用這個名字
SparseSam3Model = Sam3Model
# └─ ROI-PRUNE d END
